...
function PriceAlertPanel({ symbol, currentPrice }: { symbol: string; currentPrice: number }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (result.status === 'added') {
        ...
        Alert.alert('알림 권한이 필요합니다', '설정에서 CoinBeat 알림을 허용하면 목표가 도달 시 알려드릴 수 있어요.', [
  ...
  return <View style={[styles.alertCard, { backgroundColor: colors.cardElevated }]}>
...
const styles = StyleSheet.create({
  ...
  chart: { borderRadius: 22, padding: 18, marginTop: 27, boxShadow: '0px 8px 22px rgba(0,0,0,0.3)' },
  ...
  stat: { width: '48%', borderRadius: 16, padding: 14, boxShadow: '0px 4px 12px rgba(0,0,0,0.24)' },
  ...
  alertCard: { borderRadius: 20, padding: 17, marginTop: 18, boxShadow: '0px 8px 20px rgba(0,0,0,0.28)' },
  ...
  alertRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderTopWidth: StyleSheet.hairlineWidth, paddingVertical: 10, marginTop: 8 },
