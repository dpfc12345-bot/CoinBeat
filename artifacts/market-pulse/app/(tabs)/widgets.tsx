...
import { widgetCurrencyLabels, widgetCurrencyValues, type WidgetCurrency } from '@/src/widgets/preferences';
...
export default function WidgetsScreen() {
  ...
  const chooseCurrency = (currency: WidgetCurrency) => {
    void savePreferences({ ...preferences, currency }).then(() => setSaved(true));
  };
  ...
  return <Screen>
    <ScreenHeader eyebrow="COINBEAT WIDGETS" title="위젯" onPress={() => router.push('/settings')} />
    ...
    <View style={[styles.previewWell, { borderColor: colors.border, backgroundColor: colors.muted }]}>
      {kind === 'price'
        ? <PriceWidget assets={selectedAssets} size={size} colorTheme={preferences.colorTheme} fontSize={preferences.fontSize} design={preferences.priceDesign} currency={preferences.currency} />
    ...
    {kind === 'price' && <>
      <Text style={[styles.label, { color: colors.mutedForeground }]}>가격 표시 통화</Text>
      <View style={styles.segmentRow}>
        {widgetCurrencyValues.map((currencyOption) => <Segment
          key={currencyOption}
          icon={currencyOption === 'USD' ? 'logo-usd' : 'cash-outline'}
          label={widgetCurrencyLabels[currencyOption]}
          active={preferences.currency === currencyOption}
          onPress={() => chooseCurrency(currencyOption)}
        />)}
      </View>
    ...
    <View style={[styles.summary, { borderColor: colors.border, backgroundColor: colors.card }]}>
      <View style={styles.summaryCopy}>
        ...
        <Text style={[styles.summaryTitle, { color: colors.foreground }]}>{kind === 'price' ? selectedLabel : '주요 뉴스'} · {sizeLabels[size]} 위젯 · {widgetColorThemeLabels[preferences.colorTheme]} · 글자 {widgetFontSizeLabels[preferences.fontSize]}{kind === 'price' ? ` · ${widgetCurrencyLabels[preferences.currency]}` : ''}</Text>
      </View>
...
/** Mirrors MarketPriceWidget's priceFontSize: shrinks long prices so every card in a row keeps the same height. */
function previewPriceFontSize(asset: MarketAsset, currency: WidgetCurrency, base: number) {
...
function PriceAssetCard({ asset, theme, f, currency, showName }: { asset: MarketAsset; theme: WidgetThemeColors; f: (base: number) => number; currency: WidgetCurrency; showName?: boolean }) {
  return <View style={[styles.priceCell, { backgroundColor: theme.surface, borderColor: theme.border, width: '100%' }]}>
    ...
    <Text numberOfLines={1} style={[styles.widgetPrice, { color: theme.foreground, fontSize: f(previewPriceFontSize(asset, currency, 12)) }]}>{formatPrice(asset, currency)}</Text>
  </View>;
}

/** Mirrors the real Android widget's own theme/font handling, so this preview matches the home screen result exactly. */
function PriceWidget({ assets, size, colorTheme, fontSize, design, currency }: { assets: MarketAsset[]; size: WidgetSize; colorTheme: WidgetColorTheme; fontSize: WidgetFontSize; design: PriceWidgetDesign; currency: WidgetCurrency }) {
  ...
  if (design === 'beacon') {
    ...
    return <View style={[styles.widgetCard, widgetSizes[size], { backgroundColor: theme.background, borderColor: theme.border }]}>
      ...
      {lead && <View style={[styles.beaconHero, { backgroundColor: theme.surface, borderColor: theme.border }]}>
        ...
        <Text style={[styles.beaconPrice, { color: theme.foreground, fontSize: f(24) }]}>{formatPrice(lead, currency)}</Text>
      </View>}
      {size !== 'small' && rest.length > 0 && <View style={styles.priceGrid}>{rest.map((asset) => <View key={asset.symbol} style={styles.gridHalf}><PriceAssetCard asset={asset} theme={theme} f={f} currency={currency} /></View>)}</View>}
    </View>;
  }
    ...
    return <View style={[styles.widgetCard, widgetSizes[size], { backgroundColor: theme.background, borderColor: theme.border }]}>
      ...
      <View style={styles.stackList}>
        {assets.slice(0, count).map((asset, index) => <View key={asset.symbol} style={[styles.stackRow, { backgroundColor: theme.surface, borderLeftColor: asset.change24h >= 0 ? theme.positive : theme.negative }]}>
          ...
          <Text style={[styles.widgetPrice, { color: theme.foreground, fontSize: f(11), marginTop: 0 }]}>{formatPrice(asset, currency)}</Text>
  ...
  if (design === 'briefing') {
    ...
    return <View style={[styles.widgetCard, widgetSizes[size], { backgroundColor: theme.background, borderColor: theme.border }]}>
      ...
      {topMover && <View style={[styles.briefingHero, { backgroundColor: theme.surface }]}>
        ...
        <View style={styles.priceCellTop}><Text style={[styles.priceSymbol, { color: theme.foreground, fontSize: f(13) }]}>{topMover.symbol} · {formatPrice(topMover, currency)}</Text><Text style={[styles.priceChange, { color: topMover.change24h >= 0 ? theme.positive : theme.negative, fontSize: f(10) }]}>{topMover.change24h >= 0 ? '+' : ''}{topMover.change24h.toFixed(2)}%</Text></View>
      </View>}
      {size !== 'small' && grid.length > 0 && <View style={styles.priceGrid}>{grid.map((asset) => <View key={asset.symbol} style={styles.gridHalf}><PriceAssetCard asset={asset} theme={theme} f={f} currency={currency} /></View>)}</View>}
    </View>;
  }
  ...
  if (design === 'elastic') {
    if (size === 'small') {
      ...
      return <View style={[styles.widgetCard, widgetSizes[size], { backgroundColor: theme.background, borderColor: theme.border }]}>
        ...
        {lead && <View style={styles.gridHalf}><PriceAssetCard asset={lead} theme={theme} f={f} currency={currency} showName /></View>}
      </View>;
    }
    const count = size === 'large' ? 8 : 4;
    return <View style={[styles.widgetCard, widgetSizes[size], { backgroundColor: theme.background, borderColor: theme.border }]}>
      <WidgetHeader theme={theme} f={f} eyebrow="ELASTIC GRID" kicker="자동 조정" />
      <View style={styles.priceGrid}>{assets.slice(0, count).map((asset) => <View key={asset.symbol} style={styles.gridHalf}><PriceAssetCard asset={asset} theme={theme} f={f} currency={currency} /></View>)}</View>
  ...
  return <View style={[styles.widgetCard, widgetSizes[size], { backgroundColor: theme.background, borderColor: theme.border }]}>
    ...
    <View style={styles.priceGrid}>
       {visibleAssets.map((asset) => <View key={asset.symbol} style={styles.gridHalf}><PriceAssetCard asset={asset} theme={theme} f={f} currency={currency} /></View>)}
    </View>
...
function NewsWidget({ items, size, isLoading, isError, colorTheme, fontSize, design }: { items: ApiNewsItem[]; size: WidgetSize; isLoading: boolean; isError: boolean; colorTheme: WidgetColorTheme; fontSize: WidgetFontSize; design: NewsWidgetDesign }) {
  ...
  if (design === 'brief') {
    ...
    return <View style={[styles.widgetCard, widgetSizes[size], { backgroundColor: theme.background, borderColor: theme.border }]}>
      ...
      <Pressable testID={`widget-news-${lead.id}`} onPress={() => { void Linking.openURL(lead.sourceUrl); }} style={[styles.headlineHero, { backgroundColor: theme.surface }]}>
        ...
        <View style={styles.priceCellTop}><Text style={[styles.widgetSymbol, { color: theme.muted, fontSize: f(9) }]}>뉴스</Text><Text style={[styles.widgetSymbol, { color: theme.muted, fontSize: f(9) }]}>{lead.relativeTime}</Text></View>
      </Pressable>
  ...
  return <View style={[styles.widgetCard, widgetSizes[size], { backgroundColor: theme.background, borderColor: theme.border }]}>
    ...
    <View style={[styles.newsFooter, { borderTopColor: theme.border }]}><Text style={[styles.source, { color: theme.primary, fontSize: f(10) }]}>실시간 뉴스</Text><Text style={[styles.widgetSymbol, { color: theme.muted, fontSize: f(9) }]}>헤드라인을 누르면 원문 열기</Text></View>
  </View>;
}

function formatPrice(asset: MarketAsset, currency: WidgetCurrency) {
  if (currency === 'USD') {
