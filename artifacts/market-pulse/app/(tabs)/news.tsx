...
export default function NewsScreen() { const colors = useColors(); const [filter, setFilter] = useState<'ALL' | NewsCategory>('ALL'); const newsQuery = useGetNews(); const news = newsQuery.data ?? []; const filtered = filter === 'ALL' ? news : news.filter((item) => item.categories.includes(filter)); return <Screen><ScreenHeader eyebrow="CRYPTO PANIC NEWS" title="뉴스" onPress={() => router.push('/settings')} /><Text style={[styles.intro, { color: colors.mutedForeground }]}>CryptoPanic이 전하는 최신 암호화폐 헤드라인을 빠르게 확인하세요.</Text><View style={styles.filters}>{filters.map((item) => <Pressable key={item} testID={`news-filter-${item}`} onPress={() => setFilter(item)} style={[styles.filter, { backgroundColor: filter === item ? colors.primary : colors.card, borderColor: filter === item ? colors.primary : colors.border }]}><Text style={[styles.filterText, { color: filter === item ? colors.primaryForeground : colors.mutedForeground }]}>{item === 'ALL' ? '전체' : formatCategory(item)}</Text></Pressable>)}</View>{newsQuery.isError && <Text style={{ color: colors.negative, marginBottom: 16 }}>뉴스 API 연결 또는 설정을 확인하세요.</Text>}{filtered.map((item) => <NewsRow key={item.id} item={item} />)}</Screen>; }
