  ...
  return (
    <Shell className="bg-[#05070e]">
      ...
      <div className="mt-3 rounded-[22px] bg-[#080d18] p-3 shadow-[inset_0_0_0_1px_rgba(255,255,255,.06)]">
        ...
        <Preview design={design} kind={kind} size={size} scale={scale} />
      </div>
...
function Preview({ design, kind, size, scale }: { design: Design; kind: Kind; size: Size; scale: number }) {
  const compact = size === "small";
  const wide = size === "large";
  const font = (base: number) => ({ fontSize: `${base * scale}px` });
  if (kind === "news") {
    return <div className={`overflow-hidden rounded-[17px] ${design === "briefing" ? "bg-[#20151a]" : design === "ticker" ? "border border-[#7fe3ff33] bg-[#0b1b2a]" : "bg-[#10182b]"} p-3`} style={{ minHeight: compact ? 104 : wide ? 176 : 136 }}>
      <div className="flex items-center justify-between border-b border-white/[.08] pb-2"><div className="flex items-center gap-1.5"><Mark color={design === "briefing" ? "#ffad78" : "#73a8ff"} size={22} /><span className="font-extrabold" style={font(11)}>{design === "briefing" ? "오늘의 시장 브리핑" : "블록미디어 뉴스"}</span></div><Status compact /></div>
      <div className="mt-1">{news.slice(0, compact ? 1 : wide ? 3 : 2).map((item) => <NewsLine key={item.title} item={item} featured={design === "beacon" || design === "briefing"} />)}</div>
